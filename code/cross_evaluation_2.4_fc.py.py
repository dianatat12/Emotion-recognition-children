...
# Initialize wandb
wandb.init(project="cross_dataset_app2five_4fc_girls", entity="dtat-universitat-oberta-de-catalunya")
...
def load_model(model_name, num_classes, device, weights_path):
    ...
    # Define classifier layer with expected dimensions
    model.classifier = torch.nn.Sequential(
        torch.nn.BatchNorm1d(1280),
        torch.nn.Linear(1280, 640),
        torch.nn.ReLU(),
        torch.nn.BatchNorm1d(640),
        torch.nn.Linear(640, num_classes)  # Ensure num_classes matches output size
    )
    
    # Load the state_dict with strict=False to avoid issues with size mismatch in the classifier
    pretrained_dict = torch.load(weights_path, map_location=device)
    model_dict = model.state_dict()
    
    # Filter out classifier layer weights due to size mismatch
...
if __name__ == "__main__":
    ...
    weights_path = '/home/diana/Documents/pth_reselection/emoreact_boys_4fc.pth'

    # Load the model with saved weights
    print("Loading model...")
    model = load_model(model_name, num_classes, device, weights_path)
